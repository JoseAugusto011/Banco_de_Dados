...
class Cliente:
    ...
    @staticmethod
    def buscar_por_nome_email(db, nome, email):
        try:
            query = "SELECT * FROM Cliente WHERE nome = %s AND email = %s"
            db.cursor.execute(query, (nome, email))
            return db.cursor.fetchone()  # Usamos fetchone() porque esperamos apenas um cliente
        except Exception as e:
            print(e)
            return None
...
def main():
    ...
    prato4 = PratoTipico(db, "Paella", "Prato típico espanhol", 60.00, "Principal", "Espanha", True, 12)
    prato4.inserir()

    prato5 = PratoTipico(db, "Sushi", "Prato típico japonês", 45.00, "Principal", "Japão", True, 18)
    prato5.inserir()

    prato6 = PratoTipico(db, "Poutine", "Prato típico canadense", 30.00, "Principal", "Canadá", True, 25)
    prato6.inserir()

    prato7 = PratoTipico(db, "Moussaka", "Prato típico grego", 55.00, "Principal", "Grécia", True, 10)
    prato7.inserir()

    prato8 = PratoTipico(db, "Ceviche", "Prato típico peruano", 40.00, "Principal", "Peru", True, 20)
    prato8.inserir()

    prato9 = PratoTipico(db, "Kimchi", "Prato típico coreano", 30.00, "Acompanhamento", "Coreia do Sul", True, 15)
    prato9.inserir()

    prato10 = PratoTipico(db, "Moussaka", "Prato típico grego", 55.00, "Principal", "Grécia", True, 10)
    prato10.inserir()
